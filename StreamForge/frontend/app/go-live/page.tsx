    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="text-center">
          <p className="text-xl font-bold text-slate-200">Sign in to go live</p>
          <p className="text-sm text-slate-500 mt-2">You need to be logged in to start a stream.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="relative min-h-screen bg-slate-950 overflow-hidden flex items-center justify-center">
      ...
      <div className="relative w-full max-w-lg mx-4">
        <div className="mb-8 text-center">
          ...
          <p className="text-slate-400 text-sm mt-1.5">Start streaming to your audience</p>
        </div>

        <div className="bg-slate-900 rounded-3xl ring-1 ring-slate-800 p-7 shadow-2xl shadow-black/50">
          {step === 'form' && (
            <form onSubmit={handleCreate} className="space-y-5">
              <div>
                ...
                <input
                  ...
                  maxLength={200}
                  className="w-full px-4 py-2.5 rounded-xl bg-slate-800 border border-slate-700 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-red-500 text-sm"
                />
              </div>
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  maxLength={2000}
                  className="w-full px-4 py-2.5 rounded-xl bg-slate-800 border border-slate-700 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-red-500 text-sm resize-none"
                />
              </div>
          ...
          {(step === 'ready' || step === 'going-live') && streamKey && (
            <div className="space-y-6">
              <div className="bg-slate-800/60 rounded-xl p-4">
                <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Stream title</p>
                <p className="text-white font-medium">{title}</p>
              </div>

              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Stream Key</p>
                  <button onClick={() => setKeyVisible((v) => !v)} className="text-xs text-indigo-400 hover:text-indigo-300 transition-colors">
                    {keyVisible ? 'Hide' : 'Show'}
                  </button>
                </div>
                <div className="flex items-center gap-2 bg-slate-800 rounded-xl px-4 py-3 ring-1 ring-slate-700">
                  <code className="flex-1 min-w-0 text-xs text-emerald-400 font-mono break-all">
                    {keyVisible ? streamKey : '•'.repeat(streamKey.length)}
                  </code>
                  <button onClick={() => navigator.clipboard.writeText(streamKey)} className="shrink-0 text-xs text-slate-400 hover:text-slate-200 transition-colors px-2 py-1 rounded-md hover:bg-slate-700">
                    Copy
                  </button>
                </div>
                <p className="mt-1.5 text-[11px] text-slate-500">Keep this key private. Use it with OBS or any broadcasting software.</p>
              </div>
